...
import { leavescols } from "./Export2ExcelDetls";
...
const Export2ExcelLeaves = React.lazy(() => import("./Export2ExcelLeaves"));
...
const Export2ExcelLeavesDialog = ({ state, setState, onClose }) => {
  const field_width = "138";
  const [isLoad, setIsload] = useState(true);
  //const { hocperiod, setHocPeriodYrId, setHocPeriodMthId } = useHocPeriod();
  const { leavesperiodbymth, setLeavePeriodMthId,  setLeavePeriodYrId} = useLeavesPeriodbyMth()
  ...
  return (
    <Box>
      <Box p={2}>
        <Heading size="md">LEAVES EXPORT TO EXCEL DIALOG</Heading>
      </Box>
      {/* <Box>
...
export default Export2ExcelLeavesDialog;
