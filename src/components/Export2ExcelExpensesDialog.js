...
import { expensescols } from "./Export2ExcelDetls";
...
const Export2ExcelExpenses = React.lazy(() => import("./Export2ExcelExpenses"));
...
const Export2ExcelExpensesDialog = ({ state, setState, onClose }) => {
  const field_width = "138";
  const [isLoad, setIsload] = useState(true);
  const { expensesperiod, setExpPeriodYrId,  setExpPeriodMthId} = useExpensesPeriod()
  ...
  useEffect(() => {
    setExpPeriodYrId(state.year);
    setExpPeriodMthId(state.month);
    setIsload(false);
  }, [isLoad]);
  ...
  return (
    <Box>
      <Box p={2}>
        <Heading size="md">EXPENSES EXPORT TO EXCEL DIALOG</Heading>
      </Box>
      {/* <Box>
      ...
      <Divider />
      <Box p={2}>
        <Export2ExcelExpenses
          filename={state.filename}
          dataset={expensesperiod}
          title={state.title}
          colstype={expensescols}
          datatype={state.type}
        />
      </Box>
      {/* <div>
...
export default Export2ExcelExpensesDialog;
